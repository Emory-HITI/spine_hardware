...
def process_config(json_file):
    ...
    config.summary_dir = os.path.join("experiments", config.exp_name, "results/")
    config.log_dir = os.path.join("experiments", config.exp_name, "logs/")
    create_dirs([config.summary_dir, config.log_dir])

    # setup logging in the project
    setup_logging(config.log_dir)

    logging.getLogger().info("Hi, This is root.")
    logging.getLogger().info("All the configurations are successfully processed and dirs are created.")
